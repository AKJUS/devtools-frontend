...
var PerformanceTraceContext = class _PerformanceTraceContext extends ConversationContext {
  ...
  async getSuggestions() {
    ...
    if (insightSet) {
      ...
      const top3FailingInsightSuggestions = Object.values(insightSet.model).filter((model) => model.state !== "pass" && !(model instanceof Error)).map((model) => new PerformanceInsightFormatter(focus, model).getSuggestions().at(-1)).filter((suggestion) => !!suggestion).slice(0, 3);
